...
-- glassmorphism + hover auto patch safe for library loadstring mobile desktop

local TweenService = game:GetService("TweenService")
local Lighting = game:GetService("Lighting")

local blur = Lighting:FindFirstChild("GlassBlur")
if not blur then
...
local function applyGlass(obj)
    ...
    if obj.BackgroundTransparency < 1 then
        obj.BackgroundTransparency = math.clamp(obj.BackgroundTransparency + 0.15,0,0.6)
    end
...
local function addHover(obj)
    ...
    local function enter()
        TweenService:Create(obj,TweenInfo.new(0.15,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{
            ...
            BackgroundTransparency = math.clamp(baseTrans + 0.05,0,1)
        }):Play()
        if stroke then
    ...
    local function leave()
        ...
        if stroke then
            TweenService:Create(stroke,TweenInfo.new(0.15),{Transparency=0.85}):Play()
        end
    end
